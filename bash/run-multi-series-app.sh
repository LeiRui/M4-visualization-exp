...
do
  ...
  $HOME_PATH/query_experiment_multiseries_app.sh ${DEVICE} ${MEASUREMENT} ${TIMESTAMP_PRECISION} ${DATA_MIN_TIME} ${DATA_MAX_TIME} ${FIX_QUERY_RANGE} $nts mac >> result_${i}.txt
...
do
  ...
  $HOME_PATH/query_experiment_multiseries_app.sh ${DEVICE} ${MEASUREMENT} ${TIMESTAMP_PRECISION} ${DATA_MIN_TIME} ${DATA_MAX_TIME} ${FIX_QUERY_RANGE} $nts cpv >> result_${i}.txt
